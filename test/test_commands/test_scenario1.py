...
def test_yaml_it():
    with chdir_to_file_dir(__file__):
        ...
        shutil.rmtree(str(Path(__file__).parent / "scenario1/.out"), ignore_errors=True)
        shutil.rmtree(str(Path(__file__).parent / "scenario1/uncompiled/.bash2gitlab"), ignore_errors=True)
